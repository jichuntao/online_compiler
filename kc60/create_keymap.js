...
var KeycodeToKey = require('./keycodeToKey.json');
...
function create_keymap_file(data)
{
	...
	try
	{
		...
		keymap_Data += '#include "../tmk_keyboard/keyboard/'+corePath+'/keymap_common.h"\n\n';
		keymap_Data += keymap_common(matrix);
		keymap_Data += keymap_layer(matrix,layers,fnArr,macroArr);
		keymap_Data += keymap_speicalKey();
		var fnStr = handleFunction(fnArr,macroArr);
		var macroStr = handleMacro(macroArr);
		keymap_Data += macroStr;
		keymap_Data += fnStr;
	}
	catch(e){
		ret.status='error';
...
function handleMacro(macroArr)
{
	if(macroArr.length<=0){
		return '';
	}
	var ret = 'enum macro_id {\n';
	var i=0;
	ret+=tableStr(4);
	for(i=0;i<macroArr.length;i++){
		ret+='macro_'+i+',';
	}
	ret += '\n};\n\n';
	ret += 'const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt)\n{\n';
	ret += tableStr(4)+'switch (id) {\n';
	for(i=0;i<macroArr.length;i++){
		ret+=tableStr(8)+ 'case macro_'+i+':\n';
		ret+=tableStr(12)+'return (record->event.pressed ? MACRO(';
		if(!macroArr[i].length){
			continue;
		}
		for(var j=0;j<macroArr[i].length;j++){
			var key=macroArr[i][j];
			if(KeycodeToKey[key[0]]!=key[2]){
				continue;
			}
			ret+=' '+key[1]+'('+key[2]+'),';
		}
	...
	ret+='\n';
	ret+=tableStr(4)+'}\n';
	ret+=tableStr(4)+'return MACRO_NONE;\n';
	ret+='}\n\n'
	return ret;
}

function handleFunction(fnArr,macroArr)
{
	...
	for(i=0;i<fnArr.length;i++){
		...
		if(typeof(fn)=='string'){
			...
			else if(fncfg.type==7){
				fnOutArr[i].args[0]='macro_'+pushMacroData(fndata,macroArr);
			}
		}
		else{
...
function handleKey(key,fnArr,macroArr)
{
	...
	if(key[0]==''){
		...
		pkey='FN'+pushFnData(key[1],fnArr,macroArr);
	}
	else if(FnType.fntype_sp[key[0]]){
		pkey='FN'+pushFnData(key[0],fnArr,macroArr);
	}
	else{
		pkey=getkey(key[0]);
	}
...
function pushMacroData(macroData,macroArr)
{
	var ret=-1;
	for(var i=0;i<macroArr.length;i++){
		var obj = macroArr[i];
		if(JSON.stringify(obj)==JSON.stringify(macroData)){
			ret=i;
			break;
		}
	}
...
function keymap_layer(matrix,layers,fnArr,macroArr)
{
	...
	for(i=0;i<layers.length;i++)
	{
		...
		for(j=0;j<layers[i].length;j++)
		{
			...
			for(k=0;k<rows.length;k++)
			{
				var key=rows[k];
				var cfg=matrix[j][k];
				layer[cfg.r][cfg.c]=handleKey(key,fnArr,macroArr);
			}
		}
		mlayers.push(layer);
	}
